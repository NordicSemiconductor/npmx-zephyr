...
/**
 * @brief Function for being run as thread function for kicking off a thread.
...
/**
 * @brief Callback function to be used when the reset pin toggle.
 *
 * @param[in] dev  Unused.
 * @param[in] cb   Unused.
...
void reset_callback(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
	...
	ARG_UNUSED(cb);
	ARG_UNUSED(pins);
	LOG_INF("Reset after %d ms.", k_uptime_get_32() - warning_time);
}
...
void configure_reset_interrupt(void)
{
	...
	if (!device_is_ready(irq_res.port)) {
		LOG_ERR("Error: irq_res device %s is not ready.", irq_res.port->name);
		return;
	}

	ret = gpio_pin_configure_dt(&irq_res, GPIO_INPUT);
	if (ret != 0) {
		LOG_ERR("Error %d: failed to configure %s pin %d.", ret, irq_res.port->name,
			irq_res.pin);
		return;
	}
